use donna_core::model::{ModelDefinition, ModelRegistry};
...
pub use openai_compatible::{OpenAiCompatibleProvider, OpenAiWireChatMessage};
...
impl Display for AiError {
    fn fmt(&self, formatter: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            AiError::MissingSecret(model_id) => {
                write!(formatter, "AI model {model_id} is missing auth material")
            }
