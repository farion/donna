...
const EMBEDDED_MINIMAL_SYSTEM_PROMPT: &str =
    include_str!("../../../assets/prompts/minimal_system.md");
const EMBEDDED_TASK_SAFETY_PREAMBLE: &str =
    include_str!("../../../assets/prompts/tasks/safety_preamble.md");
const EMBEDDED_UNTRUSTED_HEADER: &str =
    include_str!("../../../assets/prompts/tasks/untrusted_header.md");
const EMBEDDED_UNTRUSTED_SECTION_TEMPLATE: &str =
    include_str!("../../../assets/prompts/tasks/untrusted_section_template.md");
...
pub fn compose_task_prompt(task_prompt: &LoadedPrompt, untrusted: &[UntrustedContent]) -> String {
    let mut prompt = String::from(EMBEDDED_TASK_SAFETY_PREAMBLE);
    ...
    prompt.push_str(EMBEDDED_UNTRUSTED_HEADER);
    for section in untrusted {
        prompt.push_str(&EMBEDDED_UNTRUSTED_SECTION_TEMPLATE.replace("{label}", &section.label));
...
fn embedded_or_minimal(embedded: Option<&'static str>, notice: Option<String>) -> LoadedPrompt {
    ...
    LoadedPrompt {
        content: EMBEDDED_MINIMAL_SYSTEM_PROMPT.to_owned(),
        source: PromptSource::Embedded("minimal_system.md"),
        notice,
    }
}
...
mod tests {
    use super::{
        EMBEDDED_MINIMAL_SYSTEM_PROMPT, EMBEDDED_TASK_SAFETY_PREAMBLE,
        EMBEDDED_TODO_REMINDER_TASK_PROMPT, EMBEDDED_UNTRUSTED_HEADER,
        EMBEDDED_UNTRUSTED_SECTION_TEMPLATE, PromptSource, UntrustedContent, compose_task_prompt,
    ...
    fn system_prompt_falls_back_to_embedded_then_minimal() {
        ...
        assert_eq!(minimal.content, EMBEDDED_MINIMAL_SYSTEM_PROMPT);
        assert_eq!(minimal.source, PromptSource::Embedded("minimal_system.md"));
    }
    ...
    fn todo_reminder_uses_embedded_task_prompt() {
        ...
        assert_eq!(prompt.content, EMBEDDED_TODO_REMINDER_TASK_PROMPT);
    }
    ...
    fn composed_task_prompt_keeps_safety_before_untrusted_content() {
        ...
        let safety_index = prompt.find(EMBEDDED_TASK_SAFETY_PREAMBLE).expect("safety");
        let untrusted_index = prompt.find(EMBEDDED_UNTRUSTED_HEADER).expect("untrusted");
        let expected_section = EMBEDDED_UNTRUSTED_SECTION_TEMPLATE.replace("{label}", "mail");

        assert!(safety_index < untrusted_index);
        assert!(prompt.contains(expected_section.trim()));
    }
}
