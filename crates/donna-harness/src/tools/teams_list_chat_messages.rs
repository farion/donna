...
pub(super) fn execute(store: &LocalStore, call: &ModelToolCall) -> String {
    ...
    match store.list_teams_messages(after, before, None, limit) {
        Ok(messages) if messages.is_empty() => "No synced Teams chat messages matched.".to_owned(),
        Ok(messages) => {
            let mut output = format!("Found {} synced Teams chat messages:", messages.len());
        ...
        Err(error) => format!("I cannot list synced Teams chat messages: {error}"),
    }
}
