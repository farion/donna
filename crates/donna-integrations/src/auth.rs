...
use crate::{github_copilot_auth, microsoft, openai_auth};
use donna_config::{AppConfig, ConfigError, ModelConfig};
use std::error::Error;
    ...
    match choice {
        ...
        AuthProviderChoice::OpenAi => configure_secret_model(
            ...
            SecretModelDefaults {
                ...
                auth_kind: SecretAuthKind::OpenAi,
            },
        )?,
        ...
        AuthProviderChoice::GithubCopilot => configure_secret_model(
            ...
            SecretModelDefaults {
                ...
                auth_kind: SecretAuthKind::GithubCopilot,
            },
        )?,
    }
...
fn configure_secret_model(
    ...
    match auth_method {
        SecretAuthMethod::Browser => match defaults.auth_kind {
            SecretAuthKind::OpenAi => openai_auth::run_browser_login(secret_store, &secret_ref)
                .map_err(AuthWizardError::Message)?,
            SecretAuthKind::GithubCopilot => {
                github_copilot_auth::run_device_login(secret_store, &secret_ref)
                    .map_err(AuthWizardError::Message)?
            }
        },
        SecretAuthMethod::ApiKey => {
    ...
    println!(
        "Saved {} model metadata to {} and stored auth material in OS secret storage at {secret_ref}.",
        defaults.label,
        config_path.display()
    );
    Ok(())
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum SecretAuthMethod {
    Browser,
    ApiKey,
}

fn prompt_secret_auth_method(kind: SecretAuthKind) -> Result<SecretAuthMethod, AuthWizardError> {
    match kind {
        SecretAuthKind::OpenAi => {
            println!("Choose OpenAI auth method:");
            println!("  1) ChatGPT Plus/Pro browser login");
            println!("  2) Manually enter API key");
            loop {
                let value = prompt("Auth method [1]")?;
                match value.trim().to_ascii_lowercase().as_str() {
                    "" | "1" | "browser" | "chatgpt" | "web" => {
                        return Ok(SecretAuthMethod::Browser);
                    }
                    "2" | "api" | "api-key" | "key" => return Ok(SecretAuthMethod::ApiKey),
                    "q" | "quit" | "cancel" => return Err(AuthWizardError::Cancelled),
                    _ => println!("Enter 1, 2, or q to cancel."),
                }
            }
        }
        SecretAuthKind::GithubCopilot => Ok(SecretAuthMethod::Browser),
    }
}
...
mod tests {
    ...
    use donna_config::{AppConfig, ModelConfig};
