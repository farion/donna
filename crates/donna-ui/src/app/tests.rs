use super::*;
use donna_config::{ModelConfig, UiThemeMode};
use donna_storage::{NewAttentionItem, NewTodo, SearchQuery, StoredMemory};
...
#[test]
fn create_todo_tool_persists_due_at() {
    let tool_reply = "{\"tool\":\"create_todo\",\"arguments\":{\"title\":\"file receipts\",\"severity\":\"middle\",\"due_at\":1893456000}}";
    let (_config_dir, mut harness) = app_harness_with_config(Vec2::new(720.0, 480.0), |config| {
        config.ai.chat.selected_model = "mock-tool".to_owned();
        config.ai.models.push(ModelConfig {
            id: "mock-tool".to_owned(),
    ...
    submit_text(&mut harness, "add file receipts");
    harness.run_steps(8);

    let store = harness.state().store.as_ref().expect("store");
    let todos = store.open_todos(10).expect("open todos");
    assert_eq!(todos.len(), 1);
    assert_eq!(todos[0].title, "file receipts");
    assert_eq!(todos[0].due_at, Some(1_893_456_000));
}

#[test]
fn update_todo_due_at_tool_changes_existing_todo() {
    let tool_reply =
        "{\"tool\":\"update_todo_due_at\",\"arguments\":{\"todo_id\":1,\"due_at\":1893456000}}";
    let (_config_dir, mut harness) = app_harness_with_config(Vec2::new(720.0, 480.0), |config| {
        config.ai.chat.selected_model = "mock-tool".to_owned();
        config.ai.models.push(ModelConfig {
            id: "mock-tool".to_owned(),
    ...
    submit_text(&mut harness, "make file receipts due then");
    harness.run_steps(8);

    let store = harness.state().store.as_ref().expect("store");
    assert_eq!(
        store.todo(todo.id).expect("updated todo").due_at,
        Some(1_893_456_000)
    );
}
...
fn changechar_persists_embedded_avatar_character() {
    ...
    let config = donna_config::AppConfig::load_at(config_path).expect("reload config");
...
fn theme_command_updates_context_and_persists_without_chat_messages() {
    ...
    for (input, mode, preference, notice) in [
        ...
        let persisted = donna_config::AppConfig::load_at(&config_path).expect("reload config");
...
#[test]
fn arrow_keys_cycle_submitted_input_history() {
    let (_config_dir, mut harness) = app_harness(Vec2::new(720.0, 480.0));

    submit_text(&mut harness, "/missing-one");
    submit_text(&mut harness, "/missing-two");
    harness.state_mut().input = "draft".to_owned();
    harness.run_steps(1);

    harness.key_press(egui::Key::ArrowUp);
    harness.run_steps(1);
    assert_eq!(harness.state().input, "/missing-two");

    harness.key_press(egui::Key::ArrowUp);
    harness.run_steps(1);
    assert_eq!(harness.state().input, "/missing-one");
    ...
    harness.key_press(egui::Key::ArrowDown);
    harness.run_steps(1);
    assert_eq!(harness.state().input, "draft");
}

#[test]
fn submitted_history_ignores_empty_input() {
    let (_config_dir, mut harness) = app_harness(Vec2::new(720.0, 480.0));

    submit_text(&mut harness, "   ");
    harness.run_steps(1);
